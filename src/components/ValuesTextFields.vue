	...
	methods: {
			...
			if (this.$refs.form.value) {
				axios({
					...
					.then((response) => {
						...
						this.$emit('add-value', response.data)
					});
			}
		},
		edit: function() {
		...
		saveEdit: function() {
			...
			if (this.$refs.form.value) {
				axios({
					method: 'put',
					url: `${apiUrl}value/${this.fieldObj.id}/edit`,
					data: {
						field_value: this.fieldObj.field_value,
					}
				})
		...
		delete: function() {
			...
			axios({
				method: 'delete',
				url: `${apiUrl}value/${this.fieldObj.id}/delete`,
			})
				.then(() => {
					this.$emit('delete-value', this.fieldObj.id)
				});
		},
